...
export class EditEvent implements OnInit {
        ...
        this.eventService.get_EventByID(id).subscribe(
            (response: any) => {
                ...
                if (eventObj) {
                    ...
                    this.event.attendees = Array.isArray(participantsArr)
                        ? participantsArr.map((p: any) => p.userID)
                        : [];
                }
            },
            (error: any) => {
    ...
    private getTimeOptionValue(hours: number, minutes: number, seconds: number = 0) {
        return (
            this.timeOptions.find(
                (t) => t.value.hours === hours && t.value.minutes === minutes
    ...
    saveEvent() {
        ...
        const save$ = this.eventId
            ? this.eventService.updateEvent(resolvedEventId, eventForSubmit, participants)
            : this.eventService.put_NewEvent(eventForSubmit, participants);
